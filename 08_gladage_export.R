...
gladage_export_processing <- function(selected_imports,param) {
    ...
    #MASTER <- readRDS(file = "../MASTER_gladage.rds")
    ...
    inv_os_last_x_days <- cstivduedate_cstlist %>%
        mutate(
            upto_30_days_flag = case_when( difftime(Sys.Date() , `col_7_adj` , units="days") < (330) ~ TRUE ),
    ...
    my_df_final <- right_join( inv_os_last_x_days[,c('col_1','inv_os_upto_30_days','inv_os_between_30_60_days','inv_os_morethan_60_days')] , my_df_final , by = c("col_1" = "sage_number") )
    ...
    my_df_final <- my_df_final %>% 
        ...
        select(
            "cfm_number","sage_number","Name",
            "max_Dispatch_Date","max_invoice","kg_recd_since_invoice","kg_recd_in_last_calr_mth",
            "# alloc. per year","Kg per allocation","kg_per_year","Fee per fee period","Fee_Period",
    ...
    names(my_df_final) <- c("CFM","Sage","Member","Last dispatch","Last invoice","Kg rec'd since invoice","Kg rec'd in last cal'r mth","# alloc. per year","Kg per allocation","Kg per year","Fee per fee period","Fee period","Kg rec  >=0104","F&V rec'd since 0104","Short dated Kg recd since 0104","Last Spot Check Date","Status","Inactive Date","Allocation Contact Email")
    ...
    conditionalFormatting(wb, 1, rows=1:nrow(my_df_final), cols=which_col("Last_Spot_Check_Date"), rule="=AND(VALUE(P1)>0,VALUE(P1)<DATE(YEAR(TODAY()),MONTH(TODAY())-11,1))", style = p_warn)
    conditionalFormatting(wb, 1, rows=1:nrow(my_df_final), cols=which_col("kg_recd_since_0104"), rule="=OR(M1>((((TODAY()-DATE(2020,4,1))/365)*J1)*1.25),M1<((((TODAY()-DATE(2020,4,1))/365)*J1)*0.75))", style = p_warn)
    conditionalFormatting(wb, 1, rows=1:nrow(my_df_final), cols=which_col("sage_number"), rule="=ISBLANK(B1)", style = p_warn)
    conditionalFormatting(wb, 1, rows=1:nrow(my_df_final), cols=which_col("# alloc. per year"), rule="=AND(H1=\"\",ROW(H1)<=COUNTA(A:A))", style = p_warn)
    ...
    conditionalFormatting(wb, 1, rows=1:nrow(my_df_final), cols=which_col("kg_recd_since_0104"), rule="=OR(M1>((((TODAY()-DATE(2020,4,1))/365)*J1)*1.4),M1<((((TODAY()-DATE(2020,4,1))/365)*J1)*0.6))", style = p_alarm)
    ...
    # conditionalFormatting(wb, 1, rows=1:nrow(my_df_final), cols=which_col("inv_os_morethan_60_days"), rule="=VALUE(J1)>0", style = p_alarm)
    ...
    saveWorkbook(wb, "export_temp/gladage_demo_v1.04.xlsx", overwrite = TRUE)
    list(filepath="export_temp/gladage_demo_v1.04.xlsx")
}
